...
namespace Server
{
    ...
    class Server
    {
        ...
        private List<ConnectionInfoPipe> connectionsPipe;//
              private class ConnectionInfoPipe//создаем имен канал
             {
           ...
           public byte[] buffer = new byte[BufferSize];//Буфер     
            ...
            public void StartListeningPipe()//Ждем соединения
            {
                while (true)
                {
                NamedPipeServerStream pipeServer =
                new NamedPipeServerStream("testpipe", PipeDirection.InOut);//Имя пайпа
                Console.Write("Waiting for client connection...");
                pipeServer.WaitForConnection();//Пайп ждет соединения с клиентом
